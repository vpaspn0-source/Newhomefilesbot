...
STORAGE_CHANNEL = os.getenv("STORAGE_CHANNEL")  # ID (-100123...) atau username (@channel)
FORCE_CHANNEL = os.getenv("FORCE_CHANNEL")      # ID (-100123...) atau username (@channel)

# Validasi minimal
if not all([API_ID, API_HASH, BOT_TOKEN, STORAGE_CHANNEL, FORCE_CHANNEL]):
    raise Exception("Isi semua environment variables: API_ID, API_HASH, BOT_TOKEN, STORAGE_CHANNEL, FORCE_CHANNEL")

API_ID = int(API_ID)  # API_ID harus int
...
async def send_grid(client, chat_id, code, page):
    ...
    # Kirim satu kotak (media group) max 10 video/file
    media_group = []
    for msg_id in current:
        media_group.append(InputMediaVideo(media=int(msg_id), caption=""))
