...
STORAGE_CHANNEL = os.getenv("STORAGE_CHANNEL")  # Bisa ID (-100...) atau username (@...)
FORCE_CHANNEL = os.getenv("FORCE_CHANNEL")      # Bisa ID (-100...) atau username (@...)

# Cek semua env sudah diisi
if not all([API_ID, API_HASH, BOT_TOKEN, STORAGE_CHANNEL, FORCE_CHANNEL]):
    raise ValueError("⚠️ Environment variable API_ID, API_HASH, BOT_TOKEN, STORAGE_CHANNEL, FORCE_CHANNEL harus diisi!")

API_ID = int(API_ID)

# Konversi channel ID ke int jika berupa angka
if STORAGE_CHANNEL.lstrip('-').isdigit():
    STORAGE_CHANNEL = int(STORAGE_CHANNEL)
if FORCE_CHANNEL.lstrip('-').isdigit():
