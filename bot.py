...
# Bisa username channel atau ID (int)
...
async def start(client, message):
    ...
    await message.reply_text(
        "✨ PREMIUM FILE BOT\n\n"
        "📤 Kirim file/video\n"
        "🔗 Lalu ketik /create untuk membuat link\n"
        "📋 /mylink untuk melihat semua link kamu"
    )

# ======= UPLOAD =======
@app.on_message(filters.private & filters.command("upload"))
async def upload_cmd(client, message):
    await message.reply_text("📤 Silakan kirim file/video yang ingin diupload.")
...
async def create(client, message):
    ...
    await message.reply_text(
        f"🔐 CODE AMAN: {code}\n\n"
        f"🔗 LINK: {link}\n\n"
        f"Simpan CODE untuk restore jika bot diganti."
    )

# ======= MYLINK =======
@app.on_message(filters.private & filters.command("mylink"))
async def mylink(client, message):
    user_id = message.from_user.id
    cursor.execute("SELECT code FROM files WHERE owner=?", (user_id,))
    rows = cursor.fetchall()
    if not rows:
