...
def restore_json():
    if os.path.exists("backup.json"):
        with open("backup.json","r") as f:
            ...
            for r in data:
                cursor.execute(
                    "INSERT OR REPLACE INTO files(code,file_ids,views,owner,total_size) VALUES(?,?,?,?,?)",
                    (r["code"], r["file_ids"], r["views"], r["owner"], r["total_size"])
                )
        db.commit()
...
# ================= MEMORY =================
user_temp = {}  # simpan file_id sementara
user_size = {}
...
async def upload(client, message):
    ...
    # Forward ke STORAGE_CHANNEL dan ambil file_id
    ...
    user_temp[user_id].append(file_id)
...
async def create(client, message):
    ...
    code = generate_code()
    file_ids_str = ",".join(user_temp[user_id])
    total_size = user_size[user_id]

    cursor.execute(
        "INSERT INTO files(code,file_ids,owner,total_size) VALUES(?,?,?,?)",
        (code, file_ids_str, user_id, total_size)
    )
    db.commit()
    backup_json()
...
async def mylink(client, message):
    user_id = message.from_user.id
    cursor.execute("SELECT code,file_ids FROM files WHERE owner=?", (user_id,))
    rows = cursor.fetchall()
    if not rows:
...
async def send_grid(client, chat_id, code, page):
    ...
    total_pages = (len(file_ids)-1)//10 + 1
    start = page*10
    end = start+10
    current_files = file_ids[start:end]

    # Kirim grid 10 video/file
    media_group = []
    for file_id in current_files:
        media_group.append(InputMediaVideo(media=file_id, caption=""))  # bisa tambahkan caption jika perlu
